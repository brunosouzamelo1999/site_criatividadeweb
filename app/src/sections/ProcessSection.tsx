      ...
      <div className="max-w-4xl mx-auto px-6 md:px-12">
        <div ref={headlineRef} className="text-center opacity-0">
          ...
          <h2 className="font-heading text-3xl md:text-4xl lg:text-[48px] font-bold text-text-primary">
            O processo em <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00e5ff] to-[#00adb5] drop-shadow-[0_0_15px_rgba(0,229,255,0.25)]">5 fases</span>
          </h2>
        ...
        <div ref={stepsRef} className="relative mt-16 max-w-[800px] mx-auto">
          ...
          <div
            ref={pathRef}
            className="absolute left-7 top-0 bottom-0 w-0 border-l-[2px] border-dashed border-[#00adb5]/30 origin-top hidden md:block"
          />
          ...
          <div className="flex flex-col gap-6">
            {steps.map((step, i) => (
              <div
                key={i}
                className="process-step relative premium-card border-white/5 p-6 md:p-7 flex items-start gap-5 md:gap-6 opacity-0 shadow-lg hover:border-[#00adb5]/30"
              >
                {/* Phase Number */}
                <div
                  className="phase-number relative flex-shrink-0 w-12 h-12 bg-gradient-to-r from-[#00adb5] to-[#00c2cb] border border-white/10 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(0,173,181,0.2)]"
                >
                  <span className="font-heading text-sm font-bold text-[#060608]">{step.number}</span>
                </div>
                ...
                {/* Checkpoint */}
                <div className="hidden md:block absolute left-[25px] top-1/2 -translate-y-1/2 w-1.5 h-1.5 bg-[#00e5ff] rounded-full shadow-[0_0_8px_rgba(0,229,255,0.6)]" />
              </div>
            ))}
          </div>
        </div>
